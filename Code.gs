...
const HEADERS = ['Timestamp', 'Name', 'Reception Attendance', 'Reception Guests', 'Holy Matrimony Attendance', 'Holy Matrimony Guests', 'Message'];
...
function doPost(e) {
  ...
  // Only present for guests invited to the Holy Matrimony (their link
  // includes "&access=full") — blank for reception-only submissions.
  const holmatAttendance = (params.holmatAttendance || '').toString().trim();
  const holmatGuests = params.holmatGuests ? (parseInt(params.holmatGuests, 10) || 0) : '';
  const message = (params.message || '').toString().trim();

  sheet.appendRow([new Date(), name, attendance, guests, holmatAttendance, holmatGuests, message]);
